...
class CrissCrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"x:{x.device}")
        if x.is_cuda:
            # print("INF_TMP to cuda:0")
            INF_TMP = INF_TMP.to("cuda:0")

        energy_H = (torch.bmm(proj_query_H, proj_key_H) + INF_TMP).view(m_batchsize, width, height, height).permute(0,
                                                                                                                    2,
                                                                                                                    1,
                                                                                                                    3)
